...
Privacy seems to be the hardest problem to achieve. Especially where each transaction is saved on public database. There are some propositions on how to solve the problem. First one is to allow each voter to pick one authorization code which can then be attached into transaction making it valid. Method which allows to achieve exactly this is called ANDOS(All-or-nothing-disclosure-of-secrets) ~\cite{andos} ~\cite{salomaa1990secret} ~\cite{applied_cryptography}(23.9 All-or-Nothing Disclosure of Secrets) ~\cite{salomaa1990secret}. Unfortunatelly it has some unacceptable flaw i.e. each voter has to communicate with all other voters, which seems to be highly unpractical in e.g parlament election, but might be acceptable in smaller contextes [to be continued]. Another proposition on achieving privacy by uising blind signatures ~\cite{applied_cryptography}, (5.3 Blind Signatures), this protocol require some kind of interactivity. 
