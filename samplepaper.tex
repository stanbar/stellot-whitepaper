...
As oppose to ~\cite{liu2017voting}, our system doesn't require any preregistration phase, nor any knowledge of under laying blockchain technology.  Voter obtain authentication token from identity provider using one of avaiable methods like OAuth2, which allow ing process is designed in such a way that no knowledge of under laying blockchain technology is required. create a new keypair on the frontend side, not revealing the private key to the backend. Client prepare one ACID transaction that includes five operations:
