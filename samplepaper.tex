...
Since Stellar uses ed25519 signature scheme, which seems to not support blinded signatures(understood as valid signatures on encrypted transactions), we slightly modify the concept of blinencrypted signature, as a signature on the hash on transaction without knowing origin transaction., When Bob request Alice for encrypt the (n-1) transactions, he actually request for raw transaction. Verification can be done decrypting transaction would mean sending raw transaction (which can be verified by computing and comparing hashes). 
