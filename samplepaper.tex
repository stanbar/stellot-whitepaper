...
To protect the BobIssuer from signing a malicious transaction, we use the cut-and-choose technique described in section \ref{cut-and-choose}.

Since the signature on the transaction (ballot) is valid, AliceVoter can publish it directly to the stellar network, and track it with the received transaction id. 
