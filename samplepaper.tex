...
Gdansk Univerity of Technology\\
Gdansk, Poland\\
...
Alice creates \(n\) (e.g. 100) random  vote batch transctions.s \footnote{by \textit{random vote batch} we understand, batch of all possible trasactions with unique vote option on random candidate}, encrypts each of them with different key and sends them to Bob. He challenges Alice for \(n-1\) unblinded batch of transactions, so he can compare if they are the same as previously claimed, and if so, validate them, now: 
\begin{itemize}
 ...
 \item If all of them are correct, he signsasks Alice if the missing batch of transactions transaction and sends it back to Alice.
 % write why it could be brenchmatch her vote decision
 \begin{itemize}
