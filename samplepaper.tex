...
Alice create \(n\) (e.g. 1000) \textit{random vote} \footnote{By \textit{random vote} we understand vote on random option} transactions, encrypt each of them with different key and send them to Bob. He ask Alice for \(n-1\) keys, so he can decrypt and validate them, now: 
\begin{itemize}
