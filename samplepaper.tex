...
When we deep deeper it turns out that those requirements are hard to satisfy all together. Especially trustless, authorization, privacy and verifiability. There are many propositions on how to design protocol that satisfy part of them, or even all of them, but they are often highly unpractical or rely privacy on trust to authorities. There are propositions ~\cite{liu2017voting} that argue not to rely on trusted third party, by introducing inspectors. But they still rely on fact that issuer won't generate more ballots than actual eligible voters.assume that Additionally they require pre-registration phase which assume that each voter posses bitcoin account (co raczej mało prawdopodobne) and lack full verifability, because only issuer and coordinator are eligable to compute results ? tru ?


user to issuer each eligible voter will proceed with pre-registration phase that require 
