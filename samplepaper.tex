...
Alice create \textit{\(n\)n} (e.g. 1000) \textit{random vote} \footnote{By \textit{random vote} we understand vote on random option} transactions, encrypt each of them with different key and send them to Bob. He. Bob ask Alice for \(n-1\)n-1 keys, so he can decrypt and validate them, now: 
\begin{itemize}
...
Since Stellar uses ed25519 signature scheme, which seems to not support blinded signatures(understood as valid signatures on encrypted transactions), we slightly modify the concept of blind signature, as a signature on the hash on transaction without knowing origin transaction. When Bob request Alice for encrypt the (n-1) transactions, he actually request for raw transaction. Verification can be done by computing and comparing hashes. 
% SB Refer to stellar implementation details, so probably move it to other place
