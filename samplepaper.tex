...
% - production-ready: jest to rozwiązanie które ma sens wdrażania już dziś. Inne publikacje operają się albo na bitcoinie (który nie jest przystosowany do tworzenia na nim alpikacji tego typu), Ethereum który aktualnie nie radzi sobie ze skalowalnością w dodatku (z tego co się orientuje), nie da się zaimplementować algorytmu ślepego podpisu w smart contractcie bez ujawniania klucza prywatnego, więc platformy smart contractowe nic tu nie dają. Jest o wiele droższy. Co więcej inne publikacje zakładają, że kazdy głosujący posiada portfel bitcoinowy, co moim zdaniem jest mrzonką. Ten system nie wymaga od użytkownika żadnej wiedzy.

% algorytm ślepych podpisów wydaje mi się, że jest obowiązkowy, chyba każdy artykuł który chciał zachować prywatność używał ślepych podpisów, nawet w książce Schneiera jest rozdział o systemie do głosowania opartego ślepe podpisy. Więc to nie jest raczej żadna kontrybucja. Co może natomiast nią być to sama implementacja ślepych podpisów, dla schematu EdDSA, którego nie znalazłem zaimplementowanego nigdzie w internecie, a wydaje mi się, że zrobiłem spory research na ten temat. Ostatecznie udało mi się to zaimplementować z pomocą jednego z developerów Stellara, na bazie wpisu na blogu profesora zajmującego się kryptografią (który okazało się że miał błąd). Nie wiem czy jest to coś co można uznać za naukowy contribution. Na pewno rzemieślniczy contribution, bo poświęciłem na zaimplementowanie tego algorytmu ok. 30h :D

% - privacy-first - cały protokół jest stworzony do okoła prywatności użytkownika. Cokolwiek by się nie działo, prywatność użytkownika jest zachowana. (no chyba, że algorytm ślepych podpisów jest dziurawy)
...
it turns out that they are hard to be satisfied all together. Especially trustless, authorization, privacy, and verifiability with trustless. 
% JS DLACZEGO
